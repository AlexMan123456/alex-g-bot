...
class EconCommand extends Subcommand {
				...
				.addSubcommand((command) => {
					return command
						...
						.addNumberOption((option) => {
							return option.setName('money').setDescription('Amount of money to deposit (leave blank to deposit all)');
						});
				})
				.addSubcommand((command) => {
					return command
						...
						.addNumberOption((option) => {
							return option.setName('money').setDescription('Amount of money to withdraw (leave blank to withdraw all)');
						});
				})
				.addSubcommand((command) => {
					return command
						...
						.addUserOption((option) => {
							return option.setName('user').setDescription('The user to get the balance of (leave blank to see your own)');
						});
				})
				.addSubcommand((command) => {
					return command.setName('daily-bonus').setDescription('Claim your daily bonus');
				})
				.addSubcommand((command) => {
					return command
						...
						.addStringOption((option) => {
							return option.setName('symbol').setDescription('The symbol to set as currency symbol').setRequired(true);
						});
				})
				.addSubcommand((command) => {
					return command
						...
						.addUserOption((option) => {
							return option.setName('user').setDescription('The user to steal money from').setRequired(true);
						});
				})
				.addSubcommand((command) => {
					return command
						...
						.addUserOption((option) => {
							return option.setName('user').setDescription('The user to pay').setRequired(true);
						})
						.addNumberOption((option) => {
							return option.setName('amount').setDescription('The amount to pay the given user').setRequired(true);
						});
				})
				.addSubcommand((command) => {
					return command.setName('birthday-bonus').setDescription("Claim your bonus if it's your birthday");
				});
		});
	}
	...
	async chatInputDeposit(interaction) {
		try {
			const { money_current: previousCurrent, money_savings: previousSavings } = await getUserAndGuildRelation(
				interaction.user.id,
	...
	async chatInputDailyBonus(interaction) {
		try {
			...
			const embed = new EmbedBuilder()
				...
				.setAuthor({ name: interaction.user.globalName })
				.setColor('Green')
				.addFields({ name: 'Current', value: `${currency_symbol}${previousCurrent} → ${currency_symbol}${newCurrent}` });
	...
	async chatInputSteal(interaction) {
		try {
			...
			if (chance > 50) {
				const { cooldown_expiry } = await postCommandCooldown(
					'economy steal',
					interaction.user.id,
					interaction.guild.id,
				...
				const embedCaught = new EmbedBuilder()
					...
					.setAuthor({ name: interaction.user.globalName })
					.addFields({
						name: `You've been caught trying to steal from ${userToStealFrom.globalName}`,
						value: `You're under arrest until ${expiryDate}, ${expiryTime}. Your sentence ends <t:${new Date(cooldown_expiry.getTime() / 1000).getTime()}:R>.`
					})
					.setColor('Red');
			...
			const embedSuccess = new EmbedBuilder()
				...
				.addFields(
					{
						name: `${interaction.user.globalName}: Current`,
						value: `${currency_symbol}${oldCurrentOfUserStealing} → ${currency_symbol}${newCurrentOfUserStealing}`
					},
					{
						name: `${userToStealFrom.globalName}: Current`,
						value: `${currency_symbol}${oldCurrentOfUserToStealFrom} → ${currency_symbol}${newCurrentOfUserToStealFrom}`
					}
				)
				.setColor('Green');
		...
		try {
			...
			const embed = new EmbedBuilder()
				...
				.addFields(
					{
						name: `${interaction.user.globalName}: Current`,
						value: `${currency_symbol}${oldUserPayingCurrent} → ${currency_symbol}${newUserPayingCurrent}`
					},
					{
						name: `${userToPay.globalName}: Current`,
						value: `${currency_symbol}${oldUserToPayCurrent} → ${currency_symbol}${newUserToPayCurrent}`
					}
				)
				.setColor('Green');
		...
		try {
			...
			if (date_of_birth?.getDate() === new Date().getDate() && date_of_birth?.getMonth() === new Date().getMonth()) {
				...
				const embed = new EmbedBuilder()
					...
					.addFields({ name: 'Current', value: `${currency_symbol}${oldCurrent} → ${currency_symbol}${newCurrent}` })
					.setColor('Green');

				await postCommandCooldown(
					'economy birthday-bonus',
					interaction.user.id,
					interaction.guild.id,
			...
			await interaction.reply(
				stripIndents(`Don't be so greedy, <@${interaction.user.id}>! It's not your birthday yet - wait your turn!
